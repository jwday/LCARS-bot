...
client.on("message", async message => {
	...
	// if (!message.channel.id === '718907750645760030') {  // #music-callouts in LCARS_Test
...
async function joinVoice(message, serverQueue) {
	const voiceChannel = message.member.voice.channel;
	var connection = await voiceChannel.join();
	// randJoinSound = pickRandSound(joinSounds);
	// const dispatcher = connection.play(joinSounds + randJoinSound, { volume: 0.5 });
	connection.play(joinSounds + "gbl.hi_bot.wav", { volume: 0.5 });
	await new Promise(r => setTimeout(r, 1000)); // Sleep?
	return connection;
}

async function leaveVoice(message, serverQueue) {
	if (!serverQueue) {
		var connection = await voiceChannel.join();
		queueConstruct.connection = connection;
	}
	// randLeaveSound = pickRandSound(leaveSounds);
	// const dispatcher = serverQueue.connection.play(leaveSounds + randLeaveSound, { volume: 0.5 });
	const dispatcher = serverQueue.connection.play(leaveSounds + "gbl.bye_bot.wav", { volume: 0.5 });
...
async function yardsale(message) {
	const voiceChannel = message.member.voice.channel;
	try {
		var connection = await voiceChannel.join();
		const dispatcher = connection.play(soundsDir + "yardsale.WAV", { volume: 0.5 });
		dispatcher.on("finish", () => {
			dispatcher.end();
			// voiceChannel.leave();
		});
...
async function rockout(message) {
	const voiceChannel = message.member.voice.channel;
	try {
		var connection = await voiceChannel.join();
		const dispatcher = connection.play(soundsDir + "MA2.wav", { volume: 0.5 });
		dispatcher.on("finish", () => {
			dispatcher.end();
			// voiceChannel.leave();
		});
...
function pickRandSound(soundsFolder) {
	var soundFiles = fs.readdirSync(soundsFolder, (err, files) => {
		files.forEach(file => {
